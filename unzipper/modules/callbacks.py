...
async def unzipper_cb(client, query: CallbackQuery, texts):
    ...
    # ── Extract ───────────────────────────────────────────────────────
        ...
        try:
            ...
            if splitted_data[1] == "url":
                ...
                rchat = r_(message.forward_origin.chat if hasattr(message.forward_origin, "chat") else None) if message.forward_origin else None
            ...
            if arc_ext.replace(".", "").isnumeric():
                ...
                if splitted_data[2] == "with_pass":
                    from unzipper.client.listener import wait_for_message
                    await unzip_client.send_message(query.message.chat.id, texts["ask_password"])
                    try:
                        pwd_msg  = await wait_for_message(query.message.chat.id, timeout=60)
                        password = pwd_msg.text
                        await pwd_msg.delete()
                    except Exception:
                        return await unzip_client.answer_query(query, "**Timeout! Try again.**")
            ...
            if not files:
                # Fallback: get files from download_path but exclude the archive itself
                all_files = await get_files(download_path)
                files = [f for f in all_files if not f == arc_name]
            if not files:
                return await unzip_client.answer_query(query, texts["failed_extract"])
    ...
    # ── Upload single file ────────────────────────────────────────────
        ...
        if not files:
            all_files = await get_files(f"{Config.DOWNLOAD_LOCATION}/{uid}")
            files     = [f for f in all_files if not f.endswith(('.zip','.rar','.7z','.tar','.gz','.zst'))]
        if not files:
    ...
    # ── Upload all files ──────────────────────────────────────────────
        ...
        if not paths:
            all_files = await get_files(f"{Config.DOWNLOAD_LOCATION}/{uid}")
            paths     = [f for f in all_files if not f.endswith(('.zip','.rar','.7z','.tar','.gz','.zst'))]
        if not paths:
    ...
    # ── Gofile settings ───────────────────────────────────────────────
        ...
        if mode == "set":
            from unzipper.client.listener import wait_for_message
            await unzip_client.send_message(query.message.chat.id, texts["ask_gofile_token"])
            try:
                tkn = await wait_for_message(query.message.chat.id, timeout=60)
