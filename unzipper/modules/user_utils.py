...
# FIX: Operator precedence — wrap the whole incoming+private+regex group with ()
@unzip_client.on_message(
    (filters.incoming & filters.private & filters.regex(dl_regex)) | filters.document
)
@unzip_client.handle_erros
async def extract_dis_archive(_, message: Message, texts):
    unzip_msg     = await message.reply(texts["processing"], reply_parameters=message.id)
    user_id       = message.from_user.id
    download_path = f"{Config.DOWNLOAD_LOCATION}/{user_id}"
    is_url        = message.text and match(dl_regex, message.text)
    is_doc        = message.document

    # ── Splitted archive handling ─────────────────────────────────────
    ...
    if is_spl:
        ...
        if is_url:
            async with ClientSession() as ses:
                ...
                await unzip_client.send_message(
                    Config.LOGS_CHANNEL,
                    texts["log"].format(user_id, "N/A", "N/A", file_name, fsize)
                )
        ...
        await unzip_msg.edit(
            texts["ok_download"].format(file_name, TimeFormatter(round(e_time - s_time) * 1000))
        )
        return
...
async def extracted_dis_spl_archive(_, message: Message, texts):
    spl_umsg = await message.reply(texts["processing"], reply_parameters=message.id)
    user_id  = message.from_user.id
