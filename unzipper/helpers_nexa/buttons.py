...
def _strip_emoji_tags(text: str) -> str:
    ...
    Also removes any leftover partial/malformed emoji tags."""
    # Remove complete <emoji id=...>fallback</emoji> tags
    text = _re.sub(r'<emoji\s+id=\d+>(.*?)</emoji>', r'\1', text)
    # Remove any leftover opening tags like <emoji id=...>
    text = _re.sub(r'<emoji[^>]*>', '', text)
...
def make_btn(text, callback_data=None, url=None,
    ...
    # Always strip <emoji> tags from button text.
    # <emoji id=...> HTML works only in message text, NOT in inline button labels.
    # Even when ButtonStyle is supported (newer Pyrogram), button text must be plain.
...
class Unzipper_Buttons:
    ...
    async def make_files_keyboard(self, files: list, user_id: int, chat_id: int, inlude_files: bool = True):
        ...
        rows = [[
            ...
            make_btn(f"{E_CROSS} {STRINGS['buttons']['cancel']}",     callback_data="cancel_dis",                  icon_custom_emoji_id=ICON_CANCEL,  style=_P()),
        ]]
        if inlude_files:
            file_btns = []
    ...
    def HELP(self):
        return InlineKeyboardMarkup([
            ...
            [
                make_btn(f" {E_ARROW} ʙᴀᴄᴋ ", callback_data="megoinhome", icon_custom_emoji_id=ICON_HOME, style=_P()),
            ],
        ])
    ...
    def EXTRACT_FILE(self):
        return InlineKeyboardMarkup([
            ...
            [make_btn(f" {E_CROSS} ᴄᴀɴᴄᴇʟ ",                  callback_data="cancel_dis",                     icon_custom_emoji_id=ICON_CANCEL,   style=_P())],
        ])
    ...
    def CLEAN(self):
        return InlineKeyboardMarkup([
            [make_btn(f" {E_STOP} ᴄʟᴇᴀɴ ᴍʏ ꜰɪʟᴇs ", callback_data="cancel_dis", icon_custom_emoji_id=ICON_TRASH,  style=_P())],
            [make_btn(f" {E_SPARK} ɴᴏᴏᴏ 😳! ",        callback_data="nobully",    icon_custom_emoji_id=ICON_CHECK,  style=_P())],
        ])
    ...
    def SETTINGS_GOFILE(self):
        return InlineKeyboardMarkup([
            [
                ...
                make_btn(f" {E_CROSS} ᴅᴇʟ ᴛᴏᴋᴇɴ ",     callback_data="gf_setting-del", icon_custom_emoji_id=ICON_CANCEL, style=_P()),
            ],
